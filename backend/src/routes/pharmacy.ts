import { Router, Response } from 'express'
import { PrismaClient } from '@prisma/client'
import { authenticate, AuthRequest } from '../middleware/auth'

const router = Router()
const prisma = new PrismaClient()
router.use(authenticate)

router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    const orders = await prisma.pharmacyOrder.findMany({
      where: { clinicId: req.clinicId }, orderBy: { createdAt: 'desc' },
      include: { patient: { select: { name: true, patientCode: true } } }
    })
    res.json({ success: true, orders })
  } catch { res.status(500).json({ success: false, message: 'Server error' }) }
})

router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    const count = await prisma.pharmacyOrder.count({ where: { clinicId: req.clinicId } })
    const rxNumber = `RX-${String(count + 1).padStart(4, '0')}`
    const order = await prisma.pharmacyOrder.create({
      data: { ...req.body, clinicId: req.clinicId, rxNumber }
    })
    res.status(201).json({ success: true, order })
  } catch { res.status(500).json({ success: false, message: 'Server error' }) }
})

router.patch('/:id/dispense', async (req: AuthRequest, res: Response) => {
  try {
    await prisma.pharmacyOrder.updateMany({
      where: { id: req.params.id, clinicId: req.clinicId },
      data: { status: 'dispensed', dispensedAt: new Date() }
    })
    res.json({ success: true })
  } catch { res.status(500).json({ success: false, message: 'Server error' }) }
})

export default router
